...
def orcamentos_separacao(empresas: int = 50):
    ...
    WITH SERVICOCALCULADO AS (
        ...
        WHERE SI.DT_REGISTRO BETWEEN CURRENT_TIMESTAMP-1 AND CURRENT_TIMESTAMP
    ...
    WHERE OS.ST_ORDEMSERVICO = 'A'
        ...
        AND OS.DT_EMISSAO BETWEEN CURRENT_DATE - 30 AND CURRENT_DATE
    ...
    PLAN SORT (JOIN (JOIN (JOIN (JOIN (JOIN (JOIN (OS INDEX (ORDEMSERVICO_IDX40, ORDEMSERVICO_IDX2),
    PC INDEX (PK_PESSOA), SI INDEX (RSERVICOITEM_ORDEMSERVICO),M INDEX (PK_MARCA), PM INDEX (PK_PESSOA),PV INDEX (PK_PESSOA), I INDEX (PK_ITEM),
    IL INDEX (PK_ITEMLOCAL), LE INDEX (PK_LOCALESTOQUE)), EO INDEX (EXTEND_ORDERMSERVICO_IDX1)),
    ICO INDEX (PK_ITEMCONFERENCIAORDEM)), JOIN (ML INDEX (PK_MODELOVEICULO), MV INDEX (PK_MARCAVEICULO),
