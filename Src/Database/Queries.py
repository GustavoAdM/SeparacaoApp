...
def orcamentos_separacao(empresas: int = 50):
    ...
    PLAN SORT (HASH (JOIN (JOIN (JOIN (JOIN (JOIN (JOIN (JOIN (JOIN (HASH (JOIN (OS INDEX (ORDEMSERVICO_IDX40, ORDEMSERVICO_IDX5),
    PC INDEX (PK_PESSOA), SI INDEX (RSERVICOITEM_ORDEMSERVICO), PV INDEX (PK_PESSOA),
    PM INDEX (PK_PESSOA), I INDEX (PK_ITEM)), M NATURAL), IL INDEX (PK_ITEMLOCAL),
