    ...
    SELECT
        ...
        FROM (
            WITH SERVICOCALCULADO AS (
                ...
                WHERE SI.DT_REGISTRO BETWEEN CURRENT_TIMESTAMP-3 AND CURRENT_TIMESTAMP
            ...
            PLAN SORT (JOIN (JOIN (JOIN (JOIN (JOIN (OS INDEX (ORDEMSERVICO_IDX40, ORDEMSERVICO_IDX2), PC INDEX (PK_PESSOA), SI INDEX (RSERVICOITEM_ORDEMSERVICO), PV INDEX (PK_PESSOA), I INDEX (PK_ITEM), IL INDEX (PK_ITEMLOCAL), LE INDEX (PK_LOCALESTOQUE)), EO INDEX (EXTEND_ORDERMSERVICO_IDX1)), ICO INDEX (PK_ITEMCONFERENCIAORDEM)), M INDEX (PK_MARCA)), IRQ INDEX (IDX001_ITEMREQUISICAOOFICINA)))
            ...
            PLAN SORT (HASH (JOIN (JOIN (JOIN (JOIN (JOIN (OS INDEX (ORDEMSERVICO_IDX40, ORDEMSERVICO_IDX2), PC INDEX (PK_PESSOA), SI INDEX (RSERVICOITEM_ORDEMSERVICO), PV INDEX (PK_PESSOA), I INDEX (PK_ITEM), IL INDEX (PK_ITEMLOCAL), LE INDEX (PK_LOCALESTOQUE)), EO INDEX (EXTEND_ORDERMSERVICO_IDX1)), ICO INDEX (PK_ITEMCONFERENCIAORDEM)), M INDEX (PK_MARCA)), IRQ INDEX (IDX001_ITEMREQUISICAOOFICINA))))
        )Y
